...
run "minimal_request" {
  ...
  assert {
    condition     = msgraph_resource.this.url == "users/test-parent-id/deviceEnrollmentConfigurations"
...
run "invalid_enum" {
  ...
  variables {
    user_id                              = "test-parent-id"
    device_enrollment_configuration_type = "__graphform_invalid_enum__"
  }
