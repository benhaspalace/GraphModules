...
run "minimal_request" {
  ...
  assert {
    condition     = alltrue([for key in ["activities", "bundles", "createdByUser", "description", "following", "lastModifiedByUser", "name", "parentReference", "sharePointIds"] : !contains(keys(msgraph_resource.this.body), key)])
