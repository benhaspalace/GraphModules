...
Lifecycle: `POST /groups/{group-id}/sites/{site-id}/analytics/itemActivityStats`, `GET/PATCH/DELETE /groups/{group-id}/sites/{site-id}/analytics/itemActivityStats/{itemActivityStat-id}`.

## Usage

```hcl
module "graph_resource" {
  source = "./sites-and-lists/groups/by-group-id/sites/by-site-id/analytics/item-activity-stats"
...
| `activities` | `activities` | `list(object({       odata_type = optional(string, "#microsoft.graph.itemActivity")       access = optional(any)       activityDateTime = optional(string)       driveItem = optional(any)     }))` | no | no |
