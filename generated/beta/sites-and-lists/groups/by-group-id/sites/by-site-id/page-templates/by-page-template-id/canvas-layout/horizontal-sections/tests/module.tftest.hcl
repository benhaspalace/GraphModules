...
run "minimal_request" {
  ...
  assert {
    condition     = msgraph_resource.this.url == "groups/test-parent-id/sites/test-parent-id/pageTemplates/test-parent-id/canvasLayout/horizontalSections"
...
run "invalid_enum" {
  ...
  variables {
    ...
    site_id          = "test-parent-id"
    page_template_id = "test-parent-id"
    emphasis         = "__graphform_invalid_enum__"
  }
