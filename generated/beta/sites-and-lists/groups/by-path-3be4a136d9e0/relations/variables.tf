...
variable "group_id1" {
  ...
  validation {
    condition     = length(trimspace(var.group_id1)) > 0
    error_message = "group_id1 must not be empty."
  }
}
...
variable "term_id1" {
  ...
  validation {
    condition     = length(trimspace(var.term_id1)) > 0
    error_message = "term_id1 must not be empty."
  }
}
...
variable "relationship" {
  ...
  validation {
    condition     = var.relationship == null ? true : contains(["pin", "reuse"], var.relationship)
