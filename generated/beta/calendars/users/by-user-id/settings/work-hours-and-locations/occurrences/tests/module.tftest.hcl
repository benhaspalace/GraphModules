...
run "minimal_request" {
  ...
  assert {
    condition     = msgraph_resource.this.url == "users/test-parent-id/settings/workHoursAndLocations/occurrences"
    error_message = "The collection URL must include parent identifiers and exclude the API-version prefix."
  }

  assert {
    condition     = alltrue([for key in ["end", "placeId", "recurrenceId", "start", "timeOffDetails", "workLocationType"] : !contains(keys(msgraph_resource.this.body), key)])
...
run "invalid_enum" {
  ...
  variables {
    user_id            = "test-parent-id"
    work_location_type = "__graphform_invalid_enum__"
  }
