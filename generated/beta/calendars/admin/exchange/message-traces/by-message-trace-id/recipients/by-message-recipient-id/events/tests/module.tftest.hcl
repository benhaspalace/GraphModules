...
run "minimal_request" {
  ...
  variables {
    ...
    message_recipient_id = "test-parent-id"
  }

  assert {
    condition     = msgraph_resource.this.url == "admin/exchange/messageTraces/test-parent-id/recipients/test-parent-id/events"
...
run "invalid_enum" {
  ...
  variables {
    message_trace_id     = "test-parent-id"
    message_recipient_id = "test-parent-id"
    event_type           = "__graphform_invalid_enum__"
  }
