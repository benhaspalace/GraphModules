...
run "minimal_request" {
  ...
  assert {
    condition     = msgraph_resource.this.url == "solutions/backupRestore/siteProtectionUnits"
    error_message = "The collection URL must include parent identifiers and exclude the API-version prefix."
  }

  assert {
    condition     = alltrue([for key in ["backupRetentionPeriodInDays", "billingPolicyId", "createdBy", "createdDateTime", "error", "lastModifiedBy", "lastModifiedDateTime", "offboardRequestedDateTime", "policyId", "protectionSources", "siteId", "status"] : !contains(keys(msgraph_resource.this.body), key)])
