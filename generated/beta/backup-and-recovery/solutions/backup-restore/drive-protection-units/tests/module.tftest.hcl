...
run "minimal_request" {
  ...
  assert {
    condition     = msgraph_resource.this.url == "solutions/backupRestore/driveProtectionUnits"
    error_message = "The collection URL must include parent identifiers and exclude the API-version prefix."
  }

  assert {
    condition     = alltrue([for key in ["backupRetentionPeriodInDays", "billingPolicyId", "createdBy", "createdDateTime", "directoryObjectId", "error", "lastModifiedBy", "lastModifiedDateTime", "offboardRequestedDateTime", "policyId", "protectionSources", "status"] : !contains(keys(msgraph_resource.this.body), key)])
...
run "invalid_enum" {
  ...
  variables {
    protection_sources = "__graphform_invalid_enum__"
  }
