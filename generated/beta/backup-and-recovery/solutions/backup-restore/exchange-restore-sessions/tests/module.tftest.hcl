...
run "minimal_request" {
  ...
  assert {
    condition     = alltrue([for key in ["completedDateTime", "createdBy", "createdDateTime", "error", "granularMailboxRestoreArtifacts", "lastModifiedBy", "lastModifiedDateTime", "mailboxRestoreArtifacts", "mailboxRestoreArtifactsBulkAdditionRequests", "policyId", "restoreJobType", "restoreSessionArtifactCount", "status"] : !contains(keys(msgraph_resource.this.body), key)])
