...
variable "mailboxes" {
  description = "The list of email addresses that are added to the corresponding Exchange restore session in a bulk operation."
  type        = list(string)
  default     = null
}
...
variable "odata_type" {
  ...
  type        = string
  default     = "#microsoft.graph.mailboxRestoreArtifactsBulkAdditionRequest"
  nullable    = false
}
