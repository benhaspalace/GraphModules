...
variable "access_review_instance_decision_item_id1" {
  description = "The unique identifier of accessReviewInstanceDecisionItem"
  type        = string
  nullable    = false

  validation {
    condition     = length(trimspace(var.access_review_instance_decision_item_id1)) > 0
    error_message = "access_review_instance_decision_item_id1 must not be empty."
  }
}
