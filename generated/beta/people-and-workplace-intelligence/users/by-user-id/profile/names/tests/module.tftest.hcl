...
run "minimal_request" {
  ...
  assert {
    condition     = msgraph_resource.this.url == "users/test-parent-id/profile/names"
    error_message = "The collection URL must include parent identifiers and exclude the API-version prefix."
  }

  assert {
    condition     = alltrue([for key in ["allowedAudiences", "createdBy", "createdDateTime", "displayName", "first", "source", "inference", "initials", "isSearchable", "languageTag", "last", "lastModifiedBy", "lastModifiedDateTime", "maiden", "middle", "nickname", "pronunciation", "sources", "suffix", "title"] : !contains(keys(msgraph_resource.this.body), key)])
