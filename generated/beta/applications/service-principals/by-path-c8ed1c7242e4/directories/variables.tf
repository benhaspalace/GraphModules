...
variable "synchronization_template_id" {
  description = "The unique identifier of synchronizationTemplate"
  type        = string
  nullable    = false

  validation {
    condition     = length(trimspace(var.synchronization_template_id)) > 0
    error_message = "synchronization_template_id must not be empty."
  }
}
