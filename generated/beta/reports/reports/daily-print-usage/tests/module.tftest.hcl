...
run "minimal_request" {
  ...
  assert {
    condition     = msgraph_resource.this.url == "reports/dailyPrintUsage"
    error_message = "The collection URL must include parent identifiers and exclude the API-version prefix."
  }

  assert {
    condition     = alltrue([for key in ["blackAndWhitePageCount", "colorPageCount", "completedBlackAndWhiteJobCount", "completedColorJobCount", "completedJobCount", "doubleSidedSheetCount", "incompleteJobCount", "mediaSheetCount", "pageCount", "singleSidedSheetCount", "usageDate"] : !contains(keys(msgraph_resource.this.body), key)])
