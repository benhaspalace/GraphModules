...
run "minimal_request" {
  ...
  variables {
    ...
    sales_quote_id      = "test-parent-id"
    sales_quote_line_id = "test-parent-id"
  }

  assert {
    condition     = msgraph_resource.this.url == "financials/companies/test-parent-id/salesQuotes/test-parent-id/salesQuoteLines/test-parent-id/item/picture"
