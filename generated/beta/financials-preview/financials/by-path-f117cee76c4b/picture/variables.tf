...
variable "sales_quote_id" {
  description = "The unique identifier of salesQuote"
  type        = string
  nullable    = false

  validation {
    condition     = length(trimspace(var.sales_quote_id)) > 0
    error_message = "sales_quote_id must not be empty."
  }
}

variable "sales_quote_line_id" {
  description = "The unique identifier of salesQuoteLine"
  type        = string
  nullable    = false

  validation {
    condition     = length(trimspace(var.sales_quote_line_id)) > 0
    error_message = "sales_quote_line_id must not be empty."
  }
}
