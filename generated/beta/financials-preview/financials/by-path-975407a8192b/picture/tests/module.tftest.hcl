...
run "minimal_request" {
  ...
  variables {
    ...
    customer_payment_id         = "test-parent-id"
  }

  assert {
    condition     = msgraph_resource.this.url == "financials/companies/test-parent-id/customerPaymentJournals/test-parent-id/customerPayments/test-parent-id/customer/picture"
