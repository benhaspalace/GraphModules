...
variable "customer_payment_journal_id" {
  description = "The unique identifier of customerPaymentJournal"
  type        = string
  nullable    = false

  validation {
    condition     = length(trimspace(var.customer_payment_journal_id)) > 0
    error_message = "customer_payment_journal_id must not be empty."
  }
}

variable "customer_payment_id" {
  description = "The unique identifier of customerPayment"
  type        = string
  nullable    = false

  validation {
    condition     = length(trimspace(var.customer_payment_id)) > 0
    error_message = "customer_payment_id must not be empty."
  }
}
