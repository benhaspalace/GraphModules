...
run "minimal_request" {
  ...
  variables {
    ...
    sales_credit_memo_id      = "test-parent-id"
    sales_credit_memo_line_id = "test-parent-id"
  }

  assert {
    condition     = msgraph_resource.this.url == "financials/companies/test-parent-id/salesCreditMemos/test-parent-id/salesCreditMemoLines/test-parent-id/item/picture"
