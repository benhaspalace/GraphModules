...
variable "sales_invoice_id" {
  description = "The unique identifier of salesInvoice"
  type        = string
  nullable    = false

  validation {
    condition     = length(trimspace(var.sales_invoice_id)) > 0
    error_message = "sales_invoice_id must not be empty."
  }
}

variable "sales_invoice_line_id" {
  description = "The unique identifier of salesInvoiceLine"
  type        = string
  nullable    = false

  validation {
    condition     = length(trimspace(var.sales_invoice_line_id)) > 0
    error_message = "sales_invoice_line_id must not be empty."
  }
}
