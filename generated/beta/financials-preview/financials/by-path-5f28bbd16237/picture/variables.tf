...
variable "sales_order_id" {
  description = "The unique identifier of salesOrder"
  type        = string
  nullable    = false

  validation {
    condition     = length(trimspace(var.sales_order_id)) > 0
    error_message = "sales_order_id must not be empty."
  }
}

variable "sales_order_line_id" {
  description = "The unique identifier of salesOrderLine"
  type        = string
  nullable    = false

  validation {
    condition     = length(trimspace(var.sales_order_line_id)) > 0
    error_message = "sales_order_line_id must not be empty."
  }
}
