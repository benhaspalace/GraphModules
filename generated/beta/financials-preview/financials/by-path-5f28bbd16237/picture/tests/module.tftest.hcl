...
run "minimal_request" {
  ...
  variables {
    ...
    sales_order_id      = "test-parent-id"
    sales_order_line_id = "test-parent-id"
  }

  assert {
    condition     = msgraph_resource.this.url == "financials/companies/test-parent-id/salesOrders/test-parent-id/salesOrderLines/test-parent-id/item/picture"
