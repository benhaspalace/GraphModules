...
run "minimal_request" {
  ...
  assert {
    condition     = msgraph_resource.this.url == "users/test-parent-id/distributionLists"
    error_message = "The collection URL must include parent identifiers and exclude the API-version prefix."
  }

  assert {
    condition     = alltrue([for key in ["categories", "createdDateTime", "displayName", "lastModifiedDateTime", "notes"] : !contains(keys(msgraph_resource.this.body), key)])
