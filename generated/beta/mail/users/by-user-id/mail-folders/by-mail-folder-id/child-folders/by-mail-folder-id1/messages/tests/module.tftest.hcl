...
run "minimal_request" {
  ...
  variables {
    ...
    mail_folder_id1 = "test-parent-id"
  }

  assert {
    condition     = msgraph_resource.this.url == "users/test-parent-id/mailFolders/test-parent-id/childFolders/test-parent-id/messages"
    error_message = "The collection URL must include parent identifiers and exclude the API-version prefix."
  }

  assert {
    condition     = alltrue([for key in ["attachments", "bccRecipients", "body", "bodyPreview", "categories", "ccRecipients", "conversationId", "conversationIndex", "createdDateTime", "extensions", "flag", "from", "hasAttachments", "importance", "inferenceClassification", "internetMessageId", "isDeliveryReceiptRequested", "isDraft", "isRead", "isReadReceiptRequested", "lastModifiedDateTime", "mentions", "mentionsPreview", "multiValueExtendedProperties", "parentFolderId", "receivedDateTime", "replyTo", "sender", "sentDateTime", "singleValueExtendedProperties", "subject", "toRecipients", "uniqueBody", "unsubscribeData", "unsubscribeEnabled", "webLink"] : !contains(keys(msgraph_resource.this.body), key)])
