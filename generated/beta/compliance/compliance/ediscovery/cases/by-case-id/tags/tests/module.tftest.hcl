...
run "minimal_request" {
  ...
  assert {
    condition     = msgraph_resource.this.url == "compliance/ediscovery/cases/test-parent-id/tags"
...
run "typed_request" {
  ...
  assert {
    condition     = jsonencode(msgraph_resource.this.body["childTags"]) == jsonencode([{ "@odata.type" = "#microsoft.graph.ediscovery.tag" }])
...
run "invalid_enum" {
  ...
  variables {
    case_id             = "test-parent-id"
    child_selectability = "__graphform_invalid_enum__"
  }
