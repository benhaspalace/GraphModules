...
run "minimal_request" {
  ...
  assert {
    condition     = msgraph_resource.this.url == "security/subjectRightsRequests/test-parent-id/notes"
    error_message = "The collection URL must include parent identifiers and exclude the API-version prefix."
  }

  assert {
    condition     = alltrue([for key in ["author", "content", "createdDateTime"] : !contains(keys(msgraph_resource.this.body), key)])
