...
run "minimal_request" {
  ...
  assert {
    condition     = msgraph_resource.this.url == "security/threatIntelligence/hostCookies"
    error_message = "The collection URL must include parent identifiers and exclude the API-version prefix."
  }

  assert {
    condition     = alltrue([for key in ["domain", "firstSeenDateTime", "host", "lastSeenDateTime", "name"] : !contains(keys(msgraph_resource.this.body), key)])
