...
variable "workbook_comment_reply_id" {
  description = "The unique identifier of workbookCommentReply"
  type        = string
  nullable    = false

  validation {
    condition     = length(trimspace(var.workbook_comment_reply_id)) > 0
    error_message = "workbook_comment_reply_id must not be empty."
  }
}
