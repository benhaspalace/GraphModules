...
run "minimal_request" {
  ...
  assert {
    condition     = msgraph_resource.this.url == "identity/conditionalAccess/deletedItems/policies"
    error_message = "The collection URL must include parent identifiers and exclude the API-version prefix."
  }

  assert {
    condition     = alltrue([for key in ["conditions", "deletedDateTime", "description", "displayName", "grantControls", "sessionControls", "state"] : !contains(keys(msgraph_resource.this.body), key)])
