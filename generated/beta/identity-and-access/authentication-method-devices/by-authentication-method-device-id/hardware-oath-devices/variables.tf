variable "authentication_method_device_id" {
  description = "The unique identifier of authenticationMethodDevice"
  type        = string
  nullable    = false

  validation {
    condition     = length(trimspace(var.authentication_method_device_id)) > 0
    error_message = "authentication_method_device_id must not be empty."
  }
}
