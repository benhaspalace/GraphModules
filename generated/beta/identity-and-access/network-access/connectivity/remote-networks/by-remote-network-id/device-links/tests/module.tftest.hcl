...
run "minimal_request" {
  ...
  assert {
    condition     = msgraph_resource.this.url == "networkAccess/connectivity/remoteNetworks/test-parent-id/deviceLinks"
...
run "invalid_enum" {
  ...
  variables {
    remote_network_id          = "test-parent-id"
    bandwidth_capacity_in_mbps = "__graphform_invalid_enum__"
  }
