...
run "minimal_request" {
  ...
  assert {
    condition     = msgraph_resource.this.url == "identityGovernance/accessReviews/unified/definitions"
    error_message = "The collection URL must include parent identifiers and exclude the API-version prefix."
  }

  assert {
    condition     = alltrue([for key in ["additionalNotificationRecipients", "backupReviewers", "descriptionForAdmins", "descriptionForReviewers", "displayName", "fallbackReviewers", "instanceEnumerationScope", "instances", "reviewers", "scope", "settings", "stageSettings"] : !contains(keys(msgraph_resource.this.body), key)])
