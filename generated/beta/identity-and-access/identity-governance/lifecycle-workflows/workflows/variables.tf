...
variable "is_enabled" {
  description = "Determines whether the workflow is enabled or disabled. If this setting is true, the workflow can be run on demand or on schedule when isSchedulingEnabled is true."
  type        = bool
  default     = null
}
...
variable "last_modified_by" {
  description = "The user who last modified the workflow."
  type        = any
  default     = null
}
...
variable "subject_processing_results" {
  description = "Microsoft Graph subjectProcessingResults property."
  type = list(object({
    odata_type       = optional(string, "#microsoft.graph.identityGovernance.subjectProcessingResult")
    processingStatus = optional(string)
    reprocessedRuns = optional(list(object({
...
variable "target_subject_type" {
  ...
  validation {
    condition     = var.target_subject_type == null ? true : contains(["user", "agentIdentity", "unknownFutureValue", "provisioningObject"], var.target_subject_type)
...
variable "additional_properties" {
  ...
  validation {
    condition     = can(keys(var.additional_properties)) ? alltrue([for key in keys(var.additional_properties) : !contains(["previewScope"], key)]) : false
