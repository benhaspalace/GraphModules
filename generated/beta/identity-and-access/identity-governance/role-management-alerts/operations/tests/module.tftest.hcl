...
run "minimal_request" {
  ...
  assert {
    condition     = msgraph_resource.this.url == "identityGovernance/roleManagementAlerts/operations"
    error_message = "The collection URL must include parent identifiers and exclude the API-version prefix."
  }

  assert {
    condition     = alltrue([for key in ["createdDateTime", "lastActionDateTime", "resourceLocation", "status", "statusDetail"] : !contains(keys(msgraph_resource.this.body), key)])
