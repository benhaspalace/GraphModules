...
run "minimal_request" {
  ...
  assert {
    condition     = alltrue([for key in ["activeSignInUri", "defaultInteractiveAuthenticationMethod", "displayName", "federatedIdpMfaBehavior", "isSignedAuthenticationRequestRequired", "issuerUri", "metadataExchangeUri", "nextSigningCertificate", "openIdConnectDiscoveryEndpoint", "passiveSignInUri", "passwordChangeUri", "passwordResetUri", "preferredAuthenticationProtocol", "promptLoginBehavior", "signOutUri", "signingCertificate", "signingCertificateUpdateStatus"] : !contains(keys(msgraph_resource.this.body), key)])
...
run "invalid_enum" {
  ...
  variables {
    domain_id                  = "test-parent-id"
    federated_idp_mfa_behavior = "__graphform_invalid_enum__"
  }
