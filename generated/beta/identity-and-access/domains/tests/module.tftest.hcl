...
run "minimal_request" {
  ...
  assert {
    condition     = alltrue([for key in ["authenticationType", "availabilityStatus", "federationConfiguration", "isAdminManaged", "isDefault", "isInitial", "isRoot", "isVerified", "passwordNotificationWindowInDays", "passwordValidityPeriodInDays", "sharedEmailDomainInvitations", "state", "supportedServices"] : !contains(keys(msgraph_resource.this.body), key)])
