...
run "minimal_request" {
  ...
  assert {
    condition     = alltrue([for key in ["activity", "activityDateTime", "additionalInfo", "appId", "correlationId", "detectedDateTime", "detectionTimingType", "source", "ipAddress", "keyIds", "lastUpdatedDateTime", "location", "mitreTechniqueId", "requestId", "riskDetail", "riskEventType", "riskLevel", "riskState", "servicePrincipalDisplayName", "servicePrincipalId", "tokenIssuerType"] : !contains(keys(msgraph_resource.this.body), key)])
