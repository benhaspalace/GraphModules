variable "hardware_oath_authentication_method_id" {
  description = "The unique identifier of hardwareOathAuthenticationMethod"
  type        = string
  nullable    = false

  validation {
    condition     = length(trimspace(var.hardware_oath_authentication_method_id)) > 0
    error_message = "hardware_oath_authentication_method_id must not be empty."
  }
}
