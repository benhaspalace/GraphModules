...
run "minimal_request" {
  ...
  variables {
    windows_quality_update_policy_id = "test-parent-id"
  }

  assert {
    condition     = msgraph_resource.this.url == "deviceManagement/windowsQualityUpdatePolicies/test-parent-id/assignments"
