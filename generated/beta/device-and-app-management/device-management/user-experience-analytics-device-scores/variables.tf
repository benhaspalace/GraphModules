...
variable "additional_properties" {
  ...
  validation {
    condition     = can(keys(var.additional_properties)) ? alltrue([for key in keys(var.additional_properties) : !contains(["appReliabilityScore", "batteryHealthScore", "deviceName", "endpointAnalyticsScore", "id", "manufacturer", "meanResourceSpikeTimeScore", "model", "startupPerformanceScore", "workFromAnywhereScore"], key)]) : false
