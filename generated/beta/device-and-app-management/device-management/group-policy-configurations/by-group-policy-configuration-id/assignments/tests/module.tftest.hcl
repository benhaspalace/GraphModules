...
run "minimal_request" {
  ...
  assert {
    condition     = msgraph_resource.this.url == "deviceManagement/groupPolicyConfigurations/test-parent-id/assignments"
    error_message = "The collection URL must include parent identifiers and exclude the API-version prefix."
  }

  assert {
    condition     = alltrue([for key in ["lastModifiedDateTime", "target"] : !contains(keys(msgraph_resource.this.body), key)])
