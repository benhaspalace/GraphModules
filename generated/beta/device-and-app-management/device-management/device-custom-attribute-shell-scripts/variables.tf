...
variable "custom_attribute_type" {
  description = "Represents the expected type for a macOS custom attribute script value."
  type        = string
  default     = null

  validation {
    condition     = var.custom_attribute_type == null ? true : contains(["integer", "string", "dateTime"], var.custom_attribute_type)
    error_message = "custom_attribute_type must be one of the documented enum values."
  }
}
