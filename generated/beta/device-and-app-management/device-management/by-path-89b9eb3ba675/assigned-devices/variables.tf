variable "windows_autopilot_deployment_profile_id" {
  description = "The unique identifier of windowsAutopilotDeploymentProfile"
  type        = string
  nullable    = false

  validation {
    condition     = length(trimspace(var.windows_autopilot_deployment_profile_id)) > 0
    error_message = "windows_autopilot_deployment_profile_id must not be empty."
  }
}
