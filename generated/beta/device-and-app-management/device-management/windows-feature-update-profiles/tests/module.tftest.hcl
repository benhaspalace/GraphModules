...
run "minimal_request" {
  ...
  assert {
    condition     = msgraph_resource.this.url == "deviceManagement/windowsFeatureUpdateProfiles"
    error_message = "The collection URL must include parent identifiers and exclude the API-version prefix."
  }

  assert {
    condition     = alltrue([for key in ["assignments", "createdDateTime", "deployableContentDisplayName", "description", "displayName", "endOfSupportDate", "featureUpdateVersion", "installFeatureUpdatesOptional", "installLatestWindows10OnWindows11IneligibleDevice", "lastModifiedDateTime", "roleScopeTagIds", "rolloutSettings"] : !contains(keys(msgraph_resource.this.body), key)])
...
run "typed_request" {
  ...
  assert {
    condition     = jsonencode(msgraph_resource.this.body["assignments"]) == jsonencode([{ "@odata.type" = "#microsoft.graph.windowsFeatureUpdateProfileAssignment" }])
