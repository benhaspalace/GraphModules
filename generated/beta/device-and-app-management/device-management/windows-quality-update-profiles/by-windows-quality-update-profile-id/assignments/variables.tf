variable "windows_quality_update_profile_id" {
  description = "The unique identifier of windowsQualityUpdateProfile"
  type        = string
  nullable    = false

  validation {
    condition     = length(trimspace(var.windows_quality_update_profile_id)) > 0
    error_message = "windows_quality_update_profile_id must not be empty."
  }
}
