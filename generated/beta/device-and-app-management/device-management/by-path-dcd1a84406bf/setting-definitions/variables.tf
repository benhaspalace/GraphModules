...
variable "device_management_intent_setting_category_id" {
  description = "The unique identifier of deviceManagementIntentSettingCategory"
  type        = string
  nullable    = false

  validation {
    condition     = length(trimspace(var.device_management_intent_setting_category_id)) > 0
    error_message = "device_management_intent_setting_category_id must not be empty."
  }
}
