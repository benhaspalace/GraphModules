...
run "minimal_request" {
  ...
  assert {
    condition     = msgraph_resource.this.url == "deviceManagement/resourceAccessProfiles/test-parent-id/assignments"
    error_message = "The collection URL must include parent identifiers and exclude the API-version prefix."
  }

  assert {
    condition     = alltrue([for key in ["intent", "sourceId", "target"] : !contains(keys(msgraph_resource.this.body), key)])
