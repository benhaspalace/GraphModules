...
run "minimal_request" {
  ...
  variables {
    ...
    device_enrollment_configuration_id = "test-parent-id"
  }

  assert {
    condition     = msgraph_resource.this.url == "users/test-parent-id/deviceEnrollmentConfigurations/test-parent-id/assignments"
