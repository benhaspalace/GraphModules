...
run "minimal_request" {
  ...
  assert {
    condition     = msgraph_resource.this.url == "admin/windows/updates/updatePolicies/test-parent-id/complianceChanges"
    error_message = "The collection URL must include parent identifiers and exclude the API-version prefix."
  }

  assert {
    condition     = alltrue([for key in ["createdDateTime", "isRevoked", "revokedDateTime", "updatePolicy"] : !contains(keys(msgraph_resource.this.body), key)])
