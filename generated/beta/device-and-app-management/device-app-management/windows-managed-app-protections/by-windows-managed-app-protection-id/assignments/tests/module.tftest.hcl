...
run "minimal_request" {
  ...
  variables {
    windows_managed_app_protection_id = "test-parent-id"
  }

  assert {
    condition     = msgraph_resource.this.url == "deviceAppManagement/windowsManagedAppProtections/test-parent-id/assignments"
