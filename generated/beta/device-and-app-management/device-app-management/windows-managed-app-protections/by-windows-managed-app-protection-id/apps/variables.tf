variable "windows_managed_app_protection_id" {
  description = "The unique identifier of windowsManagedAppProtection"
  type        = string
  nullable    = false

  validation {
    condition     = length(trimspace(var.windows_managed_app_protection_id)) > 0
    error_message = "windows_managed_app_protection_id must not be empty."
  }
}
