...
Lifecycle: `POST /copilot/users/{aiUser-id}/onlineMeetings/{aiOnlineMeeting-id}/aiInsights`, `GET/PATCH/DELETE /copilot/users/{aiUser-id}/onlineMeetings/{aiOnlineMeeting-id}/aiInsights/{callAiInsight-id}`.

## Usage

```hcl
module "graph_resource" {
  source = "./teamwork-and-communications/copilot/users/by-ai-user-id/online-meetings/by-ai-online-meeting-id/ai-insights"
  ai_user_id = "parent-object-id"
  ai_online_meeting_id = "parent-object-id"
}
```
...
| `ai_online_meeting_id` | URL parameter `aiOnlineMeeting-id` | `string` | yes | no |
| `action_items` | `actionItems` | `list(object({       odata_type = optional(string, "#microsoft.graph.actionItem")       ownerDisplayName = optional(string)       text = optional(string)       title = optional(string)     }))` | no | no |
| `call_id` | `callId` | `string` | no | no |
| `content_correlation_id` | `contentCorrelationId` | `string` | no | no |
