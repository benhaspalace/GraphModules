...
variable "ai_online_meeting_id" {
  description = "The unique identifier of aiOnlineMeeting"
  type        = string
  nullable    = false

  validation {
    condition     = length(trimspace(var.ai_online_meeting_id)) > 0
    error_message = "ai_online_meeting_id must not be empty."
  }
}
