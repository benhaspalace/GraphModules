...
run "minimal_request" {
  ...
  assert {
    condition     = msgraph_resource.this.url == "teamwork/teamTemplates/test-parent-id/definitions/test-parent-id/teamDefinition/schedule/shifts"
    error_message = "The collection URL must include parent identifiers and exclude the API-version prefix."
  }

  assert {
    condition     = alltrue([for key in ["createdBy", "draftShift", "isStagedForDeletion", "schedulingGroupId", "sharedShift", "userId"] : !contains(keys(msgraph_resource.this.body), key)])
