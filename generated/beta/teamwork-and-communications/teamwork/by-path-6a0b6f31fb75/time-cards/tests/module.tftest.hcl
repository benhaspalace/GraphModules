...
run "minimal_request" {
  ...
  assert {
    condition     = msgraph_resource.this.url == "teamwork/teamTemplates/test-parent-id/definitions/test-parent-id/teamDefinition/schedule/timeCards"
...
run "invalid_enum" {
  ...
  variables {
    team_template_id            = "test-parent-id"
    team_template_definition_id = "test-parent-id"
    confirmed_by                = "__graphform_invalid_enum__"
  }
