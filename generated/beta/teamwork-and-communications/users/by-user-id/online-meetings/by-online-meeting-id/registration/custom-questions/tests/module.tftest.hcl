...
run "minimal_request" {
  ...
  assert {
    condition     = msgraph_resource.this.url == "users/test-parent-id/onlineMeetings/test-parent-id/registration/customQuestions"
...
run "invalid_enum" {
  ...
  variables {
    user_id           = "test-parent-id"
    online_meeting_id = "test-parent-id"
    answer_input_type = "__graphform_invalid_enum__"
  }
