...
run "minimal_request" {
  ...
  assert {
    condition     = msgraph_resource.this.url == "solutions/virtualEvents/townhalls/test-parent-id/sessions/test-parent-id/attendanceReports/test-parent-id/attendanceRecords"
    error_message = "The collection URL must include parent identifiers and exclude the API-version prefix."
  }

  assert {
    condition     = alltrue([for key in ["attendanceIntervals", "emailAddress", "engagements", "externalRegistrationInformation", "identity", "registrantId", "registrationId", "role", "totalAttendanceInSeconds"] : !contains(keys(msgraph_resource.this.body), key)])
