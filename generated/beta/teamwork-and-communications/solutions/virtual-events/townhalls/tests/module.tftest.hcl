...
run "minimal_request" {
  ...
  assert {
    condition     = alltrue([for key in ["audience", "capacity", "coOrganizers", "createdBy", "description", "displayName", "endDateTime", "externalEventInformation", "invitedAttendees", "isInviteOnly", "isRegistrationEnabled", "isRegistrationRequired", "presenters", "registrationConfiguration", "registrations", "sessions", "settings", "startDateTime", "status"] : !contains(keys(msgraph_resource.this.body), key)])
