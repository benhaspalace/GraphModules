...
run "minimal_request" {
  ...
  assert {
    condition     = alltrue([for key in ["audience", "coOrganizers", "createdBy", "description", "displayName", "endDateTime", "externalEventInformation", "isRegistrationEnabled", "isRegistrationRequired", "presenters", "registrationConfiguration", "registrations", "sessions", "settings", "startDateTime", "status"] : !contains(keys(msgraph_resource.this.body), key)])
...
run "invalid_enum" {
  ...
  variables {
    audience = "__graphform_invalid_enum__"
  }
