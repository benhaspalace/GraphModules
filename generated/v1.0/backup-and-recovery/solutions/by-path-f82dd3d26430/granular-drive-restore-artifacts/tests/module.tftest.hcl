...
run "minimal_request" {
  ...
  assert {
    condition     = alltrue([for key in ["browseSessionId", "completionDateTime", "directoryObjectId", "restorePointDateTime", "restoredItemKey", "restoredItemPath", "restoredItemWebUrl", "startDateTime", "status", "webUrl"] : !contains(keys(msgraph_resource.this.body), key)])
