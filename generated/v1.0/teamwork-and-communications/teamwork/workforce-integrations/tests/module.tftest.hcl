...
run "minimal_request" {
  ...
  assert {
    condition     = alltrue([for key in ["apiVersion", "createdBy", "displayName", "eligibilityFilteringEnabledEntities", "encryption", "isActive", "supportedEntities", "url"] : !contains(keys(msgraph_resource.this.body), key)])
