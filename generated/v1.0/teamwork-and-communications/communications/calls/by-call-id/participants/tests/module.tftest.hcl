...
run "minimal_request" {
  ...
  assert {
    condition     = msgraph_resource.this.url == "communications/calls/test-parent-id/participants"
    error_message = "The collection URL must include parent identifiers and exclude the API-version prefix."
  }

  assert {
    condition     = alltrue([for key in ["info", "isInLobby", "isMuted", "mediaStreams", "metadata", "recordingInfo", "removedState", "restrictedExperience", "rosterSequenceNumber"] : !contains(keys(msgraph_resource.this.body), key)])
...
run "typed_request" {
  ...
  assert {
    condition     = jsonencode(msgraph_resource.this.body["isInLobby"]) == jsonencode(false)
    error_message = "isInLobby must preserve typed values and omit nested nulls."
  }
