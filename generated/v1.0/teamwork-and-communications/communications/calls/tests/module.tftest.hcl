...
run "minimal_request" {
  ...
  assert {
    condition     = msgraph_resource.this.url == "communications/calls"
    error_message = "The collection URL must include parent identifiers and exclude the API-version prefix."
  }

  assert {
    condition     = alltrue([for key in ["audioRoutingGroups", "callChainId", "callOptions", "callbackUri", "chatInfo", "contentSharingSessions", "source", "incomingContext", "mediaConfig", "meetingInfo", "myParticipantId", "operations", "participants", "requestedModalities", "subject", "targets", "tenantId", "toneInfo"] : !contains(keys(msgraph_resource.this.body), key)])
