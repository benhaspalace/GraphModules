...
run "minimal_request" {
  ...
  variables {
    ...
    engagement_conversation_message_id1       = "test-parent-id"
  }

  assert {
    condition     = msgraph_resource.this.url == "communications/onlineMeetingConversations/test-parent-id/messages/test-parent-id/replies/test-parent-id/reactions"
...
run "invalid_enum" {
  ...
  variables {
    ...
    engagement_conversation_message_id        = "test-parent-id"
    engagement_conversation_message_id1       = "test-parent-id"
    reaction_type                             = "__graphform_invalid_enum__"
  }
