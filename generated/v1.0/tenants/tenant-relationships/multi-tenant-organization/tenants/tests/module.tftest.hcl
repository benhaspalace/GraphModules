...
run "minimal_request" {
  ...
  assert {
    condition     = msgraph_resource.this.url == "tenantRelationships/multiTenantOrganization/tenants"
    error_message = "The collection URL must include parent identifiers and exclude the API-version prefix."
  }

  assert {
    condition     = alltrue([for key in ["deletedDateTime", "displayName", "role", "tenantId"] : !contains(keys(msgraph_resource.this.body), key)])
...
run "invalid_enum" {
  ...
  variables {
    role = "__graphform_invalid_enum__"
  }
