...
run "minimal_request" {
  ...
  assert {
    condition     = alltrue([for key in ["description", "faceCheckConfiguration", "lastModifiedDateTime", "name", "priority", "state", "verifiedIdProfileConfiguration", "verifiedIdUsageConfigurations", "verifierDid"] : !contains(keys(msgraph_resource.this.body), key)])
