...
variable "agreement_file_localization_id" {
  description = "The unique identifier of agreementFileLocalization"
  type        = string
  nullable    = false

  validation {
    condition     = length(trimspace(var.agreement_file_localization_id)) > 0
    error_message = "agreement_file_localization_id must not be empty."
  }
}
...
variable "additional_properties" {
  ...
  validation {
    condition     = can(keys(var.additional_properties)) ? alltrue([for key in keys(var.additional_properties) : !contains(["fileData", "fileName", "id", "isDefault", "language"], key)]) : false
