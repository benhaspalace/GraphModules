...
run "minimal_request" {
  ...
  assert {
    condition     = msgraph_resource.this.url == "identityGovernance/accessReviews/definitions/test-parent-id/instances"
    error_message = "The collection URL must include parent identifiers and exclude the API-version prefix."
  }

  assert {
    condition     = alltrue([for key in ["decisions", "fallbackReviewers", "reviewers", "stages"] : !contains(keys(msgraph_resource.this.body), key)])
