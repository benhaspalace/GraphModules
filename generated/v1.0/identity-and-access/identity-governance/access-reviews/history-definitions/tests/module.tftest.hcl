...
run "minimal_request" {
  ...
  assert {
    condition     = alltrue([for key in ["createdBy", "createdDateTime", "decisions", "displayName", "instances", "reviewHistoryPeriodEndDateTime", "reviewHistoryPeriodStartDateTime", "scheduleSettings", "scopes", "status"] : !contains(keys(msgraph_resource.this.body), key)])
