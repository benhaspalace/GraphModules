...
run "minimal_request" {
  ...
  assert {
    condition     = msgraph_resource.this.url == "identityGovernance/entitlementManagement/accessPackages/test-parent-id/assignmentPolicies"
...
run "invalid_enum" {
  ...
  variables {
    access_package_id    = "test-parent-id"
    allowed_target_scope = "__graphform_invalid_enum__"
  }
