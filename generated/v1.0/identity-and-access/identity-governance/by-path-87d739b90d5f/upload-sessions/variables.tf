variable "access_package_resource_environment_id" {
  description = "The unique identifier of accessPackageResourceEnvironment"
  type        = string
  nullable    = false

  validation {
    condition     = length(trimspace(var.access_package_resource_environment_id)) > 0
    error_message = "access_package_resource_environment_id must not be empty."
  }
}

variable "access_package_resource_id" {
  description = "The unique identifier of accessPackageResource"
  type        = string
  nullable    = false

  validation {
    condition     = length(trimspace(var.access_package_resource_id)) > 0
    error_message = "access_package_resource_id must not be empty."
  }
}

variable "access_package_resource_role_id" {
  description = "The unique identifier of accessPackageResourceRole"
  type        = string
  nullable    = false

  validation {
    condition     = length(trimspace(var.access_package_resource_role_id)) > 0
    error_message = "access_package_resource_role_id must not be empty."
  }
}
