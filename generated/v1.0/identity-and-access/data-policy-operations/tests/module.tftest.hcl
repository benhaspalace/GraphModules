...
run "minimal_request" {
  ...
  assert {
    condition     = msgraph_resource.this.url == "dataPolicyOperations"
    error_message = "The collection URL must include parent identifiers and exclude the API-version prefix."
  }

  assert {
    condition     = alltrue([for key in ["completedDateTime", "progress", "status", "storageLocation", "submittedDateTime", "userId"] : !contains(keys(msgraph_resource.this.body), key)])
