...
run "minimal_request" {
  ...
  assert {
    condition     = alltrue([for key in ["commerceSubscriptionId", "createdDateTime", "isTrial", "nextLifecycleDateTime", "ownerId", "ownerTenantId", "ownerType", "serviceStatus", "skuId", "skuPartNumber", "status", "totalLicenses"] : !contains(keys(msgraph_resource.this.body), key)])
