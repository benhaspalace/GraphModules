...
run "minimal_request" {
  ...
  assert {
    condition     = alltrue([for key in ["adDomainName", "adDomainPassword", "adDomainUsername", "alternateResourceUrl", "connectionType", "displayName", "healthCheckStatus", "organizationalUnit", "resourceGroupId", "scopeIds", "subnetId", "subscriptionId", "virtualNetworkId"] : !contains(keys(msgraph_resource.this.body), key)])
...
run "invalid_enum" {
  ...
  variables {
    connection_type = "__graphform_invalid_enum__"
  }
