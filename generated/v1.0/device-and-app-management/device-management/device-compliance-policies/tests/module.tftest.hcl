...
run "minimal_request" {
  ...
  assert {
    condition     = alltrue([for key in ["assignments", "createdDateTime", "description", "deviceSettingStateSummaries", "deviceStatusOverview", "deviceStatuses", "displayName", "version", "lastModifiedDateTime", "scheduledActionsForRule", "userStatusOverview", "userStatuses"] : !contains(keys(msgraph_resource.this.body), key)])
