...
run "minimal_request" {
  ...
  assert {
    condition     = alltrue([for key in ["apps", "assignments", "createdDateTime", "customSettings", "deployedAppCount", "deploymentSummary", "description", "displayName", "version", "isAssigned", "lastModifiedDateTime"] : !contains(keys(msgraph_resource.this.body), key)])
