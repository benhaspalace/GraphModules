variable "android_managed_app_protection_id" {
  description = "The unique identifier of androidManagedAppProtection"
  type        = string
  nullable    = false

  validation {
    condition     = length(trimspace(var.android_managed_app_protection_id)) > 0
    error_message = "android_managed_app_protection_id must not be empty."
  }
}
