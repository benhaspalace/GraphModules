...
run "minimal_request" {
  ...
  assert {
    condition     = msgraph_resource.this.url == "print/shares"
    error_message = "The collection URL must include parent identifiers and exclude the API-version prefix."
  }

  assert {
    condition     = alltrue([for key in ["allowAllUsers", "allowedGroups", "allowedUsers", "capabilities", "defaults", "displayName", "isAcceptingJobs", "jobs", "location", "manufacturer", "model", "printer", "status", "viewPoint"] : !contains(keys(msgraph_resource.this.body), key)])
