...
run "minimal_request" {
  ...
  assert {
    condition     = msgraph_resource.this.url == "users/test-parent-id/managedDevices/test-parent-id/deviceCompliancePolicyStates"
    error_message = "The collection URL must include parent identifiers and exclude the API-version prefix."
  }

  assert {
    condition     = alltrue([for key in ["displayName", "version", "platformType", "settingCount", "settingStates", "state"] : !contains(keys(msgraph_resource.this.body), key)])
...
run "typed_request" {
  ...
  assert {
    condition     = jsonencode(msgraph_resource.this.body["settingStates"]) == jsonencode([{ "@odata.type" = "#microsoft.graph.deviceCompliancePolicySettingState" }])
