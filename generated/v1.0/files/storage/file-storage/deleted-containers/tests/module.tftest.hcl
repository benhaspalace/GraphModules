...
run "minimal_request" {
  ...
  assert {
    condition     = msgraph_resource.this.url == "storage/fileStorage/deletedContainers"
    error_message = "The collection URL must include parent identifiers and exclude the API-version prefix."
  }

  assert {
    condition     = alltrue([for key in ["assignedSensitivityLabel", "columns", "customProperties", "description", "displayName", "migrationJobs", "permissions", "settings", "sharePointGroups"] : !contains(keys(msgraph_resource.this.body), key)])
