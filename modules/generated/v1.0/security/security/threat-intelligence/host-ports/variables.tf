...
variable "most_recent_ssl_certificate" {
...
variable "odata_type" {
  ...
  type        = string
  default     = "#microsoft.graph.security.hostPort"
  nullable    = false
}
...
variable "protocol" {
  ...
  validation {
    condition     = var.protocol == null ? true : contains(["tcp", "udp", "unknownFutureValue"], var.protocol)
    error_message = "protocol must be one of the documented enum values."
  }
}
...
variable "status" {
  ...
  validation {
    condition     = var.status == null ? true : contains(["open", "filtered", "closed", "unknownFutureValue"], var.status)
    error_message = "status must be one of the documented enum values."
  }
}

variable "times_observed" {
