...
variable "workbook_chart_series_id" {
  description = "The unique identifier of workbookChartSeries"
  type        = string
  nullable    = false

  validation {
    condition     = length(trimspace(var.workbook_chart_series_id)) > 0
    error_message = "workbook_chart_series_id must not be empty."
  }
}
...
variable "additional_properties" {
  ...
  validation {
    condition     = can(keys(var.additional_properties)) ? alltrue([for key in keys(var.additional_properties) : !contains(["format", "id", "value"], key)]) : false
