...
run "minimal_request" {
  ...
  assert {
    condition     = msgraph_resource.this.url == "drives/test-parent-id/items/test-parent-id/workbook/worksheets/test-parent-id/charts"
    error_message = "The collection URL must include parent identifiers and exclude the API-version prefix."
  }

  assert {
    condition     = alltrue([for key in ["height", "left", "name", "top", "width"] : !contains(keys(msgraph_resource.this.body), key)])
