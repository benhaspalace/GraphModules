variable "deleted_team_id" {
  description = "The unique identifier of deletedTeam"
  type        = string
  nullable    = false

  validation {
    condition     = length(trimspace(var.deleted_team_id)) > 0
    error_message = "deleted_team_id must not be empty."
  }
}
...
variable "importance" {
  ...
  validation {
    condition     = var.importance == null ? true : contains(["normal", "high", "urgent", "unknownFutureValue"], var.importance)
...
variable "additional_properties" {
  ...
  validation {
    condition     = can(keys(var.additional_properties)) ? alltrue([for key in keys(var.additional_properties) : !contains(["deletedDateTime", "etag", "eventDetail", "id", "lastEditedDateTime", "lastModifiedDateTime", "replyToId", "webUrl"], key)]) : false
