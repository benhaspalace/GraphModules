...
resource "msgraph_resource" "this" {
  url                     = "groups/${urlencode(var.group_id)}/sites/${urlencode(var.site_id)}/termStores/${urlencode(var.store_id)}/sets/${urlencode(var.set_id)}/children/${urlencode(var.term_id)}/children"
  api_version             = "v1.0"
  update_method           = "PATCH"
  body                    = local.body
