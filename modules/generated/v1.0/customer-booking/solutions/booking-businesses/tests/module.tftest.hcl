...
run "minimal_request" {
  ...
  assert {
    condition     = msgraph_resource.this.url == "solutions/bookingBusinesses"
    error_message = "The collection URL must include parent identifiers and exclude the API-version prefix."
  }

  assert {
    condition     = alltrue([for key in ["address", "bookingPageSettings", "businessHours", "businessType", "createdDateTime", "defaultCurrencyIso", "displayName", "email", "languageTag", "lastUpdatedDateTime", "phone", "schedulingPolicy", "webSiteUrl"] : !contains(keys(msgraph_resource.this.body), key)])
