...
variable "user_install_state_summary_id" {
  description = "The unique identifier of userInstallStateSummary"
  type        = string
  nullable    = false

  validation {
    condition     = length(trimspace(var.user_install_state_summary_id)) > 0
    error_message = "user_install_state_summary_id must not be empty."
  }
}
