...
run "minimal_request" {
  ...
  assert {
    condition     = msgraph_resource.this.url == "users/test-parent-id/managedDevices/test-parent-id/deviceConfigurationStates"
...
run "typed_request" {
  ...
  assert {
    condition     = jsonencode(msgraph_resource.this.body["settingStates"]) == jsonencode([{ "@odata.type" = "#microsoft.graph.deviceConfigurationSettingState" }])
