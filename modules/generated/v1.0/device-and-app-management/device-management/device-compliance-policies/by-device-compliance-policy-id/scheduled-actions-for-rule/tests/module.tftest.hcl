...
run "minimal_request" {
  ...
  assert {
    condition     = msgraph_resource.this.url == "deviceManagement/deviceCompliancePolicies/test-parent-id/scheduledActionsForRule"
...
run "typed_request" {
  ...
  assert {
    condition     = jsonencode(msgraph_resource.this.body["scheduledActionConfigurations"]) == jsonencode([{ "@odata.type" = "#microsoft.graph.deviceComplianceActionItem" }])
