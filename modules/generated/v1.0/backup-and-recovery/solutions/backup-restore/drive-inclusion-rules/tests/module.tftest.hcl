...
run "minimal_request" {
  ...
  assert {
    condition     = msgraph_resource.this.url == "solutions/backupRestore/driveInclusionRules"
    error_message = "The collection URL must include parent identifiers and exclude the API-version prefix."
  }

  assert {
    condition     = alltrue([for key in ["createdBy", "createdDateTime", "driveExpression", "error", "isAutoApplyEnabled", "lastModifiedBy", "lastModifiedDateTime", "status"] : !contains(keys(msgraph_resource.this.body), key)])
