...
run "minimal_request" {
  ...
  assert {
    condition     = alltrue([for key in ["createdBy", "createdDateTime", "displayName", "driveInclusionRules", "driveProtectionUnits", "driveProtectionUnitsBulkAdditionJobs", "isEnabled", "lastModifiedBy", "lastModifiedDateTime", "protectionPolicyArtifactCount", "retentionSettings", "status"] : !contains(keys(msgraph_resource.this.body), key)])
...
run "typed_request" {
  ...
  assert {
    condition     = jsonencode(msgraph_resource.this.body["driveInclusionRules"]) == jsonencode([{ "@odata.type" = "#microsoft.graph.driveProtectionRule" }])
    error_message = "driveInclusionRules must preserve typed values and omit nested nulls."
  }
}
