...
run "minimal_request" {
  ...
  assert {
    condition     = alltrue([for key in ["createdBy", "createdDateTime", "displayName", "isEnabled", "lastModifiedBy", "lastModifiedDateTime", "protectionPolicyArtifactCount", "retentionSettings", "siteInclusionRules", "siteProtectionUnits", "siteProtectionUnitsBulkAdditionJobs", "status"] : !contains(keys(msgraph_resource.this.body), key)])
