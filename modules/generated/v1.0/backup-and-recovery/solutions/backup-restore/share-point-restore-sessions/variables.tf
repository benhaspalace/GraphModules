...
variable "granular_site_restore_artifacts" {
  description = "A collection of browse session ID and item key details that can be used to restore SharePoint files and folders."
  type = list(object({
    odata_type           = optional(string, "#microsoft.graph.granularSiteRestoreArtifact")
    browseSessionId      = optional(string)
    completionDateTime   = optional(string)
    restorePointDateTime = optional(string)
    restoredItemKey      = optional(string)
    restoredItemPath     = optional(string)
    restoredItemWebUrl   = optional(string)
...
variable "site_restore_artifacts_bulk_addition_requests" {
  description = "A collection of SharePoint site URLs and destination details that can be used to restore SharePoint sites."
  type = list(object({
    odata_type             = optional(string, "#microsoft.graph.siteRestoreArtifactsBulkAdditionRequest")
