...
run "minimal_request" {
  ...
  assert {
    condition     = msgraph_resource.this.url == "servicePrincipals/test-parent-id/synchronization/templates/test-parent-id/schema/directories"
...
run "invalid_enum" {
  ...
  variables {
    service_principal_id        = "test-parent-id"
    synchronization_template_id = "test-parent-id"
    discoverabilities           = "__graphmodules_invalid_enum__"
  }
