...
resource "msgraph_resource" "this" {
  url                     = "identityGovernance/entitlementManagement/resourceEnvironments/${urlencode(var.access_package_resource_environment_id)}/resources/${urlencode(var.access_package_resource_id)}/scopes/${urlencode(var.access_package_resource_scope_id)}/resource/uploadSessions"
  api_version             = "v1.0"
  update_method           = "PATCH"
  body                    = local.body
