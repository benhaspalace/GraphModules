...
run "minimal_request" {
  ...
  assert {
    condition     = msgraph_resource.this.url == "identityGovernance/entitlementManagement/resourceEnvironments"
    error_message = "The collection URL must include parent identifiers and exclude the API-version prefix."
  }

  assert {
    condition     = alltrue([for key in ["connectionInfo", "createdDateTime", "description", "displayName", "isDefaultEnvironment", "modifiedDateTime", "originId", "originSystem"] : !contains(keys(msgraph_resource.this.body), key)])
