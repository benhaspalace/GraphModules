...
resource "msgraph_resource" "this" {
  url                     = "identityGovernance/catalogs/${urlencode(var.access_package_catalog_id)}/resources/${urlencode(var.access_package_resource_id)}/roles/${urlencode(var.access_package_resource_role_id)}/resource/scopes/${urlencode(var.access_package_resource_scope_id)}/resource/uploadSessions"
  api_version             = "v1.0"
  update_method           = "PATCH"
  body                    = local.body
