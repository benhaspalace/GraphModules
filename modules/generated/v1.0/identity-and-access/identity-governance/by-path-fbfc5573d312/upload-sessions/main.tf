...
resource "msgraph_resource" "this" {
  url                     = "identityGovernance/entitlementManagement/resourceRequests/${urlencode(var.access_package_resource_request_id)}/catalog/resourceScopes/${urlencode(var.access_package_resource_scope_id)}/resource/roles/${urlencode(var.access_package_resource_role_id)}/resource/uploadSessions"
  api_version             = "v1.0"
  update_method           = "PATCH"
  body                    = local.body
