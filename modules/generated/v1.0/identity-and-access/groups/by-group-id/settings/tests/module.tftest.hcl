...
run "minimal_request" {
  ...
  assert {
    condition     = msgraph_resource.this.url == "groups/test-parent-id/settings"
    error_message = "The collection URL must include parent identifiers and exclude the API-version prefix."
  }

  assert {
    condition     = alltrue([for key in ["displayName", "values"] : !contains(keys(msgraph_resource.this.body), key)])
