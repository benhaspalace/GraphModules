...
run "minimal_request" {
  ...
  assert {
    condition     = alltrue([for key in ["activity", "appId", "displayName", "history", "initiatedBy", "isEnabled", "isProcessing", "riskDetail", "riskLastUpdatedDateTime", "riskLevel", "riskState", "servicePrincipalType"] : !contains(keys(msgraph_resource.this.body), key)])
