...
run "minimal_request" {
  ...
  assert {
    condition     = msgraph_resource.this.url == "roleManagement/directory/resourceNamespaces/test-parent-id/resourceActions"
    error_message = "The collection URL must include parent identifiers and exclude the API-version prefix."
  }

  assert {
    condition     = alltrue([for key in ["actionVerb", "authenticationContextId", "description", "isAuthenticationContextSettable", "name", "resourceScopeId"] : !contains(keys(msgraph_resource.this.body), key)])
