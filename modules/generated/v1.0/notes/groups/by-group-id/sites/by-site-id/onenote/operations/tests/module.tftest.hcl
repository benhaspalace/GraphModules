...
run "minimal_request" {
  ...
  assert {
    condition     = msgraph_resource.this.url == "groups/test-parent-id/sites/test-parent-id/onenote/operations"
    error_message = "The collection URL must include parent identifiers and exclude the API-version prefix."
  }

  assert {
    condition     = alltrue([for key in ["createdDateTime", "error", "lastActionDateTime", "percentComplete", "resourceId", "resourceLocation", "status"] : !contains(keys(msgraph_resource.this.body), key)])
