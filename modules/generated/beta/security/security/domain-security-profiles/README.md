...
Lifecycle: `POST /security/domainSecurityProfiles`, `GET/PATCH/DELETE /security/domainSecurityProfiles/{domainSecurityProfile-id}`.

## Usage

```hcl
module "graph_resource" {
  source = "./security/security/domain-security-profiles"
}
```
...
| `count_in_org` | `countInOrg` | `number` | no | no |
| `domain_categories` | `domainCategories` | `list(object({       odata_type = optional(string, "#microsoft.graph.reputationCategory")       description = optional(string)       name = optional(string)       vendor = optional(string)     }))` | no | no |
| `domain_registered_date_time` | `domainRegisteredDateTime` | `string` | no | no |
