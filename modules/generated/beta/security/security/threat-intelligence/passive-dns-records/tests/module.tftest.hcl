...
run "minimal_request" {
  ...
  assert {
    condition     = msgraph_resource.this.url == "security/threatIntelligence/passiveDnsRecords"
    error_message = "The collection URL must include parent identifiers and exclude the API-version prefix."
  }

  assert {
    condition     = alltrue([for key in ["artifact", "collectedDateTime", "firstSeenDateTime", "lastSeenDateTime", "parentHost", "recordType"] : !contains(keys(msgraph_resource.this.body), key)])
