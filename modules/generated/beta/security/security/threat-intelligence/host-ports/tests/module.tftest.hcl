# Offline plan tests: Terraform mocks the provider and never contacts Microsoft Graph.
mock_provider "msgraph" {}

run "minimal_request" {
  command = plan

  assert {
    condition     = msgraph_resource.this.url == "security/threatIntelligence/hostPorts"
    error_message = "The collection URL must include parent identifiers and exclude the API-version prefix."
  }

  assert {
    condition     = alltrue([for key in ["banners", "firstSeenDateTime", "host", "lastScanDateTime", "lastSeenDateTime", "mostRecentSslCertificate", "port", "protocol", "services", "status", "timesObserved"] : !contains(keys(msgraph_resource.this.body), key)])
    error_message = "Unset optional inputs must be omitted from the Graph request."
  }
}

run "typed_request" {
  command = plan

  variables {
    first_seen_date_time = "2026-01-01T00:00:00Z"
    port                 = -2147483648
    host                 = { "childHostPairs" = null }
    banners              = [{}]
  }

  assert {
    condition     = jsonencode(msgraph_resource.this.body["firstSeenDateTime"]) == jsonencode("2026-01-01T00:00:00Z")
    error_message = "firstSeenDateTime must preserve typed values and omit nested nulls."
  }

  assert {
    condition     = jsonencode(msgraph_resource.this.body["port"]) == jsonencode(-2147483648)
    error_message = "port must preserve typed values and omit nested nulls."
  }

  assert {
    condition     = jsonencode(msgraph_resource.this.body["host"]) == jsonencode({ "@odata.type" = "#microsoft.graph.security.host" })
    error_message = "host must preserve typed values and omit nested nulls."
  }

  assert {
    condition     = jsonencode(msgraph_resource.this.body["banners"]) == jsonencode([{ "@odata.type" = "#microsoft.graph.security.hostPortBanner" }])
    error_message = "banners must preserve typed values and omit nested nulls."
  }
}

run "invalid_enum" {
  command = plan

  variables {
    protocol = "__graphmodules_invalid_enum__"
  }

  expect_failures = [var.protocol]
}
