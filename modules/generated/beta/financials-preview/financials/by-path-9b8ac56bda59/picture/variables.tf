...
variable "sales_credit_memo_id" {
  description = "The unique identifier of salesCreditMemo"
  type        = string
  nullable    = false

  validation {
    condition     = length(trimspace(var.sales_credit_memo_id)) > 0
    error_message = "sales_credit_memo_id must not be empty."
  }
}

variable "sales_credit_memo_line_id" {
  description = "The unique identifier of salesCreditMemoLine"
  type        = string
  nullable    = false

  validation {
    condition     = length(trimspace(var.sales_credit_memo_line_id)) > 0
    error_message = "sales_credit_memo_line_id must not be empty."
  }
}
