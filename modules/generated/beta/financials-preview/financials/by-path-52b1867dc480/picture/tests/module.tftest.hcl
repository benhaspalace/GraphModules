...
run "minimal_request" {
  ...
  variables {
    ...
    sales_invoice_id      = "test-parent-id"
    sales_invoice_line_id = "test-parent-id"
  }

  assert {
    condition     = msgraph_resource.this.url == "financials/companies/test-parent-id/salesInvoices/test-parent-id/salesInvoiceLines/test-parent-id/item/picture"
