...
run "minimal_request" {
  ...
  variables {
    ...
    purchase_invoice_id      = "test-parent-id"
    purchase_invoice_line_id = "test-parent-id"
  }

  assert {
    condition     = msgraph_resource.this.url == "financials/companies/test-parent-id/purchaseInvoices/test-parent-id/purchaseInvoiceLines/test-parent-id/item/picture"
