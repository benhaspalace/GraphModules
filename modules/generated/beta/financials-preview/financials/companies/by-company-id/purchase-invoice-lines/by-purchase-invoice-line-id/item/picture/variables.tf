...
variable "purchase_invoice_line_id" {
  description = "The unique identifier of purchaseInvoiceLine"
  type        = string
  nullable    = false

  validation {
    condition     = length(trimspace(var.purchase_invoice_line_id)) > 0
    error_message = "purchase_invoice_line_id must not be empty."
  }
}
