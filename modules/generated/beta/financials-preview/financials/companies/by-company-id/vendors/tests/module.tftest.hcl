...
run "minimal_request" {
  ...
  assert {
    condition     = msgraph_resource.this.url == "financials/companies/test-parent-id/vendors"
    error_message = "The collection URL must include parent identifiers and exclude the API-version prefix."
  }

  assert {
    condition     = alltrue([for key in ["address", "balance", "blocked", "currency", "currencyCode", "currencyId", "displayName", "email", "id", "lastModifiedDateTime", "number", "paymentMethod", "paymentMethodId", "paymentTerm", "paymentTermsId", "phoneNumber", "picture", "taxLiable", "taxRegistrationNumber", "website"] : !contains(keys(msgraph_resource.this.body), key)])
