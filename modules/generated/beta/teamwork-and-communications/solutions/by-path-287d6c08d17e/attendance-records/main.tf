...
resource "msgraph_resource" "this" {
  url                     = "solutions/virtualEvents/webinars/${urlencode(var.virtual_event_webinar_id)}/sessions/${urlencode(var.virtual_event_session_id)}/attendanceReports/${urlencode(var.meeting_attendance_report_id)}/attendanceRecords"
  api_version             = "beta"
  update_method           = "PATCH"
  body                    = local.body
