...
run "minimal_request" {
  ...
  assert {
    condition     = msgraph_resource.this.url == "solutions/approval/operations"
    error_message = "The collection URL must include parent identifiers and exclude the API-version prefix."
  }

  assert {
    condition     = alltrue([for key in ["createdDateTime", "error", "lastActionDateTime", "resourceLocation", "status"] : !contains(keys(msgraph_resource.this.body), key)])
