...
run "minimal_request" {
  ...
  assert {
    condition     = msgraph_resource.this.url == "solutions/virtualEvents/townhalls/test-parent-id/registrations"
    error_message = "The collection URL must include parent identifiers and exclude the API-version prefix."
  }

  assert {
    condition     = alltrue([for key in ["cancelationDateTime", "email", "externalRegistrationInformation", "firstName", "lastName", "preferredLanguage", "preferredTimezone", "registrantVideoOnDemandWebUrl", "registrationDateTime", "registrationQuestionAnswers", "sessions", "userId"] : !contains(keys(msgraph_resource.this.body), key)])
