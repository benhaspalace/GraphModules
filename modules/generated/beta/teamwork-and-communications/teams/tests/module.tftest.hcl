...
run "minimal_request" {
  ...
  assert {
    condition     = msgraph_resource.this.url == "teams"
    error_message = "The collection URL must include parent identifiers and exclude the API-version prefix."
  }

  assert {
    condition     = alltrue([for key in ["allChannels", "channels", "classification", "createdDateTime", "description", "discoverySettings", "displayName", "firstChannelName", "funSettings", "group", "guestSettings", "incomingChannels", "installedApps", "internalId", "isMembershipLimitedToOwners", "memberSettings", "members", "messagingSettings", "operations", "owners", "permissionGrants", "photo", "primaryChannel", "schedule", "specialization", "summary", "tags", "template", "templateDefinition", "tenantId", "visibility", "webUrl"] : !contains(keys(msgraph_resource.this.body), key)])
...
run "invalid_enum" {
  ...
  variables {
    specialization = "__graphmodules_invalid_enum__"
  }
