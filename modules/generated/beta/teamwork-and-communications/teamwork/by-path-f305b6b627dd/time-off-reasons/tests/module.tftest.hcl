...
run "minimal_request" {
  ...
  assert {
    condition     = msgraph_resource.this.url == "teamwork/teamTemplates/test-parent-id/definitions/test-parent-id/teamDefinition/schedule/timeOffReasons"
...
run "invalid_enum" {
  ...
  variables {
    team_template_id            = "test-parent-id"
    team_template_definition_id = "test-parent-id"
    icon_type                   = "__graphmodules_invalid_enum__"
  }
