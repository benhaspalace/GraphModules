...
run "minimal_request" {
  ...
  assert {
    condition     = msgraph_resource.this.url == "teamwork/teamTemplates/test-parent-id/definitions/test-parent-id/teamDefinition/schedule/timesOff"
    error_message = "The collection URL must include parent identifiers and exclude the API-version prefix."
  }

  assert {
    condition     = alltrue([for key in ["createdBy", "draftTimeOff", "isStagedForDeletion", "sharedTimeOff", "userId"] : !contains(keys(msgraph_resource.this.body), key)])
