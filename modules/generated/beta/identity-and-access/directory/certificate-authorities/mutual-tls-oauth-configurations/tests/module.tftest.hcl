...
run "minimal_request" {
  ...
  assert {
    condition     = msgraph_resource.this.url == "directory/certificateAuthorities/mutualTlsOauthConfigurations"
...
run "invalid_enum" {
  ...
  variables {
    tls_client_auth_parameter = "__graphmodules_invalid_enum__"
  }
