...
run "minimal_request" {
  ...
  assert {
    condition     = msgraph_resource.this.url == "identityGovernance/accessReviews/historyDefinitions"
    error_message = "The collection URL must include parent identifiers and exclude the API-version prefix."
  }

  assert {
    condition     = alltrue([for key in ["createdBy", "createdDateTime", "decisions", "displayName", "downloadUri", "fulfilledDateTime", "instances", "reviewHistoryPeriodEndDateTime", "reviewHistoryPeriodStartDateTime", "scheduleSettings", "scopes", "status"] : !contains(keys(msgraph_resource.this.body), key)])
...
run "typed_request" {
  ...
  assert {
    condition     = jsonencode(msgraph_resource.this.body["decisions"]) == jsonencode(["approve"])
