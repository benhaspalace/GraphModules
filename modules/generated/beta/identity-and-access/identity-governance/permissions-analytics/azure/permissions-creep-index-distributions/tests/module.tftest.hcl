...
run "minimal_request" {
  ...
  assert {
    condition     = msgraph_resource.this.url == "identityGovernance/permissionsAnalytics/azure/permissionsCreepIndexDistributions"
    error_message = "The collection URL must include parent identifiers and exclude the API-version prefix."
  }

  assert {
    condition     = alltrue([for key in ["authorizationSystem", "createdDateTime", "highRiskProfile", "lowRiskProfile", "mediumRiskProfile"] : !contains(keys(msgraph_resource.this.body), key)])
