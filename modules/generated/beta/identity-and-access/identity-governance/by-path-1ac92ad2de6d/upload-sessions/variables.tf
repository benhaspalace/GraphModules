...
variable "access_package_assignment_resource_role_id" {
  description = "The unique identifier of accessPackageAssignmentResourceRole"
  type        = string
  nullable    = false

  validation {
    condition     = length(trimspace(var.access_package_assignment_resource_role_id)) > 0
    error_message = "access_package_assignment_resource_role_id must not be empty."
  }
}
