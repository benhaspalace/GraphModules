...
run "minimal_request" {
  ...
  assert {
    condition     = msgraph_resource.this.url == "identityGovernance/accessReviews/unified/definitions/test-parent-id/instances/test-parent-id/decisions/test-parent-id/instance/stages"
    error_message = "The collection URL must include parent identifiers and exclude the API-version prefix."
  }

  assert {
    condition     = alltrue([for key in ["decisions", "fallbackReviewers", "reviewers"] : !contains(keys(msgraph_resource.this.body), key)])
