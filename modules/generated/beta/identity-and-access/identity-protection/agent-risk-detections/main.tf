# Generated from pinned Microsoft Graph sources; do not edit.
locals {
  typed_body = { for key, value in {
    "activityDateTime"        = var.activity_date_time
    "additionalInfo"          = var.additional_info
    "agentDisplayName"        = var.agent_display_name
    "agentId"                 = var.agent_id
    "blueprintId"             = var.blueprint_id
    "clientSessionId"         = var.client_session_id
    "detectedDateTime"        = var.detected_date_time
    "detectionTimingType"     = var.detection_timing_type
    "displayName"             = var.display_name
    "source"                  = var.graph_source
    "identityId"              = var.identity_id
    "identityType"            = var.identity_type
    "lastModifiedDateTime"    = var.last_modified_date_time
    "@odata.type"             = var.odata_type
    "riskDetail"              = var.risk_detail
    "riskEventType"           = var.risk_event_type
    "riskEvidence"            = var.risk_evidence
    "riskLevel"               = var.risk_level
    "riskState"               = var.risk_state
    "signInClientDisplayName" = var.sign_in_client_display_name
    "signInCorrelationId"     = var.sign_in_correlation_id
    "signInRequestId"         = var.sign_in_request_id
  } : key => value if value != null }
  body = merge({ for key, value in var.additional_properties : key => value if value != null }, local.typed_body)
}

resource "msgraph_resource" "this" {
  url                     = "identityProtection/agentRiskDetections"
  api_version             = "beta"
  update_method           = "PATCH"
  body                    = local.body
  ignore_missing_property = true

  response_export_values = {
    response = "@"
  }
}
