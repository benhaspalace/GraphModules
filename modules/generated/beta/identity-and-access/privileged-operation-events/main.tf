# Generated from pinned Microsoft Graph sources; do not edit.
locals {
  typed_body = { for key, value in {
    "additionalInformation" = var.additional_information
    "creationDateTime"      = var.creation_date_time
    "expirationDateTime"    = var.expiration_date_time
    "@odata.type"           = var.odata_type
    "referenceKey"          = var.reference_key
    "referenceSystem"       = var.reference_system
    "requestType"           = var.request_type
    "requestorId"           = var.requestor_id
    "requestorName"         = var.requestor_name
    "roleId"                = var.role_id
    "roleName"              = var.role_name
    "tenantId"              = var.tenant_id
    "userId"                = var.user_id
    "userMail"              = var.user_mail
    "userName"              = var.user_name
  } : key => value if value != null }
  body = merge({ for key, value in var.additional_properties : key => value if value != null }, local.typed_body)
}

resource "msgraph_resource" "this" {
  url                     = "privilegedOperationEvents"
  api_version             = "beta"
  update_method           = "PATCH"
  body                    = local.body
  ignore_missing_property = true

  response_export_values = {
    response = "@"
  }
}
