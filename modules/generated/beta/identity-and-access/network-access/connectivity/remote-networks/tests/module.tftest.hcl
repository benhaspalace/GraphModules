...
run "minimal_request" {
  ...
  assert {
    condition     = msgraph_resource.this.url == "networkAccess/connectivity/remoteNetworks"
    error_message = "The collection URL must include parent identifiers and exclude the API-version prefix."
  }

  assert {
    condition     = alltrue([for key in ["connectivityConfiguration", "deviceLinks", "forwardingProfiles", "version", "lastModifiedDateTime", "name", "region"] : !contains(keys(msgraph_resource.this.body), key)])
...
run "invalid_enum" {
  ...
  variables {
    region = "__graphmodules_invalid_enum__"
  }
