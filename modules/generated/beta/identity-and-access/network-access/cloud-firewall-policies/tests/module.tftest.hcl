...
run "minimal_request" {
  ...
  assert {
    condition     = msgraph_resource.this.url == "networkAccess/cloudFirewallPolicies"
...
run "typed_request" {
  ...
  assert {
    condition     = jsonencode(msgraph_resource.this.body["settings"]) == jsonencode({ "@odata.type" = "#microsoft.graph.networkaccess.cloudFirewallPolicySettings" })
