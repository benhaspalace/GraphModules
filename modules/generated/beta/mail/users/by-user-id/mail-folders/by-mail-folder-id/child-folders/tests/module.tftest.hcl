...
run "minimal_request" {
  ...
  assert {
    condition     = msgraph_resource.this.url == "users/test-parent-id/mailFolders/test-parent-id/childFolders"
    error_message = "The collection URL must include parent identifiers and exclude the API-version prefix."
  }

  assert {
    condition     = alltrue([for key in ["childFolderCount", "childFolders", "displayName", "isHidden", "messageRules", "messages", "operations", "parentFolderId", "totalItemCount", "unreadItemCount", "userConfigurations", "wellKnownName"] : !contains(keys(msgraph_resource.this.body), key)])
