variable "file_storage_container_type_id" {
  description = "The unique identifier of fileStorageContainerType"
  type        = string
  nullable    = false

  validation {
    condition     = length(trimspace(var.file_storage_container_type_id)) > 0
    error_message = "file_storage_container_type_id must not be empty."
  }
}
