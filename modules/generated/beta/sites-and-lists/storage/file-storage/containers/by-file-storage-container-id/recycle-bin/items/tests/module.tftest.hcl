...
run "minimal_request" {
  ...
  assert {
    condition     = msgraph_resource.this.url == "storage/fileStorage/containers/test-parent-id/recycleBin/items"
    error_message = "The collection URL must include parent identifiers and exclude the API-version prefix."
  }

  assert {
    condition     = alltrue([for key in ["createdByUser", "deletedDateTime", "deletedFromLocation", "description", "lastModifiedByUser", "name", "parentReference", "size"] : !contains(keys(msgraph_resource.this.body), key)])
