...
resource "msgraph_resource" "this" {
  url                     = "groups/${urlencode(var.group_id)}/sites/${urlencode(var.site_id)}/termStore/groups/${urlencode(var.group_id1)}/sets/${urlencode(var.set_id)}/children/${urlencode(var.term_id)}/children"
  api_version             = "beta"
  update_method           = "PATCH"
  body                    = local.body
