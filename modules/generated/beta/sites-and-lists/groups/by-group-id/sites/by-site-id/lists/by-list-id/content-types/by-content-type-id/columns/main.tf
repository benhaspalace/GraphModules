...
locals {
  typed_body = { for key, value in {
    ...
    "sourceContentType"     = var.source_content_type
    "term"                  = var.term
    "text"                  = var.text
...
resource "msgraph_resource" "this" {
  url                     = "groups/${urlencode(var.group_id)}/sites/${urlencode(var.site_id)}/lists/${urlencode(var.list_id)}/contentTypes/${urlencode(var.content_type_id)}/columns"
  api_version             = "beta"
  update_method           = "PATCH"
  body                    = local.body
