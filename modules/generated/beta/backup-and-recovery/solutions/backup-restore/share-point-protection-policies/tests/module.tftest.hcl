...
run "minimal_request" {
  ...
  assert {
    condition     = msgraph_resource.this.url == "solutions/backupRestore/sharePointProtectionPolicies"
    error_message = "The collection URL must include parent identifiers and exclude the API-version prefix."
  }

  assert {
    condition     = alltrue([for key in ["billingPolicyId", "createdBy", "createdDateTime", "displayName", "isEnabled", "lastModifiedBy", "lastModifiedDateTime", "offboardRequestedDateTime", "protectionMode", "protectionPolicyArtifactCount", "retentionSettings", "siteExclusionUnits", "siteExclusionUnitsBulkAdditionJobs", "siteInclusionRules", "siteProtectionUnits", "siteProtectionUnitsBulkAdditionJobs", "status"] : !contains(keys(msgraph_resource.this.body), key)])
