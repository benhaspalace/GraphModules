...
resource "msgraph_resource" "this" {
  url                     = "groups/${urlencode(var.group_id)}/team/channels/${urlencode(var.channel_id)}/planner/plans/${urlencode(var.planner_plan_id)}/tasks/${urlencode(var.planner_task_id)}/messages"
  api_version             = "beta"
  update_method           = "PATCH"
  body                    = local.body
