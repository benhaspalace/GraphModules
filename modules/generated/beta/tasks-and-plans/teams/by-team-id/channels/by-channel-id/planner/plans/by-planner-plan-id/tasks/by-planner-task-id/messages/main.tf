...
resource "msgraph_resource" "this" {
  url                     = "teams/${urlencode(var.team_id)}/channels/${urlencode(var.channel_id)}/planner/plans/${urlencode(var.planner_plan_id)}/tasks/${urlencode(var.planner_task_id)}/messages"
  api_version             = "beta"
  update_method           = "PATCH"
  body                    = local.body
