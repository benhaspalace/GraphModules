...
resource "msgraph_resource" "this" {
  url                     = "teamTemplateDefinition/${urlencode(var.team_template_definition_id)}/teamDefinition/channels/${urlencode(var.channel_id)}/planner/plans/${urlencode(var.planner_plan_id)}/buckets/${urlencode(var.planner_bucket_id)}/tasks"
  api_version             = "beta"
  update_method           = "PATCH"
  body                    = local.body
