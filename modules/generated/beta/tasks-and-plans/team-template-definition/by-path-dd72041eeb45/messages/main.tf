...
resource "msgraph_resource" "this" {
  url                     = "teamTemplateDefinition/${urlencode(var.team_template_definition_id)}/teamDefinition/primaryChannel/planner/plans/${urlencode(var.planner_plan_id)}/buckets/${urlencode(var.planner_bucket_id)}/tasks/${urlencode(var.planner_task_id)}/messages"
  api_version             = "beta"
  update_method           = "PATCH"
  body                    = local.body
