...
run "minimal_request" {
  ...
  assert {
    condition     = msgraph_resource.this.url == "compliance/ediscovery/cases/test-parent-id/legalHolds"
    error_message = "The collection URL must include parent identifiers and exclude the API-version prefix."
  }

  assert {
    condition     = alltrue([for key in ["contentQuery", "createdBy", "createdDateTime", "description", "displayName", "errors", "isEnabled", "lastModifiedBy", "lastModifiedDateTime", "siteSources", "status", "unifiedGroupSources", "userSources"] : !contains(keys(msgraph_resource.this.body), key)])
