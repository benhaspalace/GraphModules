...
run "minimal_request" {
  ...
  variables {
    ...
    workbook_document_task_id = "test-parent-id"
    workbook_comment_reply_id = "test-parent-id"
  }

  assert {
    condition     = msgraph_resource.this.url == "drives/test-parent-id/items/test-parent-id/workbook/worksheets/test-parent-id/tasks/test-parent-id/comment/replies/test-parent-id/task/changes"
