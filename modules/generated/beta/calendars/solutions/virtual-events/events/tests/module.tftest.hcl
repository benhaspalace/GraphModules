...
run "minimal_request" {
  ...
  assert {
    condition     = alltrue([for key in ["createdBy", "description", "displayName", "endDateTime", "externalEventInformation", "isRegistrationEnabled", "isRegistrationRequired", "presenters", "sessions", "settings", "startDateTime", "status"] : !contains(keys(msgraph_resource.this.body), key)])
