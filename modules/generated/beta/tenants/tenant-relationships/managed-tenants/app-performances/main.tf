...
locals {
  typed_body = { for key, value in {
    ...
    "meanTimeToFailureInMinutes" = var.mean_time_to_failure_in_minutes
    "@odata.type"                = var.odata_type
    "tenantDisplayName"          = var.tenant_display_name
    "tenantId"                   = var.tenant_id
    "totalActiveDeviceCount"     = var.total_active_device_count
...
resource "msgraph_resource" "this" {
  url                     = "tenantRelationships/managedTenants/appPerformances"
  api_version             = "beta"
  update_method           = "PATCH"
  body                    = local.body
