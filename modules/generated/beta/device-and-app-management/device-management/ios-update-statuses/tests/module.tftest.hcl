...
run "minimal_request" {
  ...
  assert {
    condition     = msgraph_resource.this.url == "deviceManagement/iosUpdateStatuses"
    error_message = "The collection URL must include parent identifiers and exclude the API-version prefix."
  }

  assert {
    condition     = alltrue([for key in ["complianceGracePeriodExpirationDateTime", "deviceDisplayName", "deviceId", "deviceModel", "installStatus", "lastReportedDateTime", "osVersion", "platform", "status", "userId", "userName", "userPrincipalName"] : !contains(keys(msgraph_resource.this.body), key)])
...
run "invalid_enum" {
  ...
  variables {
    install_status = "__graphmodules_invalid_enum__"
  }

  expect_failures = [var.install_status]
}
