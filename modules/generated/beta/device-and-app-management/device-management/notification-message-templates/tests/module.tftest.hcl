...
run "minimal_request" {
  ...
  assert {
    condition     = alltrue([for key in ["brandingOptions", "defaultLocale", "description", "displayName", "lastModifiedDateTime", "localizedNotificationMessages", "roleScopeTagIds"] : !contains(keys(msgraph_resource.this.body), key)])
...
run "invalid_enum" {
  ...
  variables {
    branding_options = "__graphmodules_invalid_enum__"
  }
