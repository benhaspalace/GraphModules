...
run "minimal_request" {
  ...
  assert {
    condition     = alltrue([for key in ["assignments", "createdDateTime", "crossRegionDisasterRecoverySetting", "displayName", "lastModifiedDateTime", "localAdminEnabled", "notificationSetting", "provisioningSourceType", "resetEnabled", "restorePointSetting", "selfServiceEnabled"] : !contains(keys(msgraph_resource.this.body), key)])
...
run "invalid_enum" {
  ...
  variables {
    provisioning_source_type = "__graphmodules_invalid_enum__"
  }

  expect_failures = [var.provisioning_source_type]
}
