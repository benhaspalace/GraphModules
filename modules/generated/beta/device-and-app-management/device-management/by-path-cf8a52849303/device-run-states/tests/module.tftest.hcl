...
run "minimal_request" {
  ...
  assert {
    condition     = msgraph_resource.this.url == "deviceManagement/deviceManagementScripts/test-parent-id/userRunStates/test-parent-id/deviceRunStates"
...
run "invalid_enum" {
  ...
  variables {
    device_management_script_id            = "test-parent-id"
    device_management_script_user_state_id = "test-parent-id"
    run_state                              = "__graphmodules_invalid_enum__"
  }
