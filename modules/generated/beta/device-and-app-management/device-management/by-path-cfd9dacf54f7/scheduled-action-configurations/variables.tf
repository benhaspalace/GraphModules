...
variable "device_management_compliance_scheduled_action_for_rule_id" {
  description = "The unique identifier of deviceManagementComplianceScheduledActionForRule"
  type        = string
  nullable    = false

  validation {
    condition     = length(trimspace(var.device_management_compliance_scheduled_action_for_rule_id)) > 0
    error_message = "device_management_compliance_scheduled_action_for_rule_id must not be empty."
  }
}
...
variable "action_type" {
  ...
  validation {
    condition     = var.action_type == null ? true : contains(["noAction", "notification", "block", "retire", "wipe", "removeResourceAccessProfiles", "pushNotification", "remoteLock"], var.action_type)
