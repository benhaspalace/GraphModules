# Generated from pinned Microsoft Graph sources; do not edit.
locals {
  typed_body = { for key, value in {
    "accountId"                 = var.account_id
    "configureWifi"             = var.configure_wifi
    "createdDateTime"           = var.created_date_time
    "description"               = var.description
    "deviceNameTemplate"        = var.device_name_template
    "displayName"               = var.display_name
    "enrolledDeviceCount"       = var.enrolled_device_count
    "enrollmentMode"            = var.enrollment_mode
    "enrollmentTokenType"       = var.enrollment_token_type
    "enrollmentTokenUsageCount" = var.enrollment_token_usage_count
    "isTeamsDeviceProfile"      = var.is_teams_device_profile
    "lastModifiedDateTime"      = var.last_modified_date_time
    "@odata.type"               = var.odata_type
    "qrCodeContent"             = var.qr_code_content
    "qrCodeImage"               = var.qr_code_image
    "roleScopeTagIds"           = (var.role_scope_tag_ids == null ? null : [for item0 in var.role_scope_tag_ids : item0 if item0 != null])
    "tokenCreationDateTime"     = var.token_creation_date_time
    "tokenExpirationDateTime"   = var.token_expiration_date_time
    "tokenValue"                = var.token_value
    "wifiHidden"                = var.wifi_hidden
    "wifiPassword"              = var.wifi_password
    "wifiSecurityType"          = var.wifi_security_type
    "wifiSsid"                  = var.wifi_ssid
  } : key => value if value != null }
  body = merge({ for key, value in var.additional_properties : key => value if value != null }, local.typed_body)
}

resource "msgraph_resource" "this" {
  url                     = "deviceManagement/androidDeviceOwnerEnrollmentProfiles"
  api_version             = "beta"
  update_method           = "PATCH"
  body                    = local.body
  ignore_missing_property = true

  response_export_values = {
    response = "@"
  }
}
