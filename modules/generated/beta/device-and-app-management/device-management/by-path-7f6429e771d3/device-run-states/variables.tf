...
variable "device_management_script_user_state_id" {
  description = "The unique identifier of deviceManagementScriptUserState"
  type        = string
  nullable    = false

  validation {
    condition     = length(trimspace(var.device_management_script_user_state_id)) > 0
    error_message = "device_management_script_user_state_id must not be empty."
  }
}
