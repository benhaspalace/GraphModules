variable "apple_user_initiated_enrollment_profile_id" {
  description = "The unique identifier of appleUserInitiatedEnrollmentProfile"
  type        = string
  nullable    = false

  validation {
    condition     = length(trimspace(var.apple_user_initiated_enrollment_profile_id)) > 0
    error_message = "apple_user_initiated_enrollment_profile_id must not be empty."
  }
}
