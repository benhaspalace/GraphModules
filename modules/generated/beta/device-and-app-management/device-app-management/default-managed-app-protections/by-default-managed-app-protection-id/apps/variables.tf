variable "default_managed_app_protection_id" {
  description = "The unique identifier of defaultManagedAppProtection"
  type        = string
  nullable    = false

  validation {
    condition     = length(trimspace(var.default_managed_app_protection_id)) > 0
    error_message = "default_managed_app_protection_id must not be empty."
  }
}
