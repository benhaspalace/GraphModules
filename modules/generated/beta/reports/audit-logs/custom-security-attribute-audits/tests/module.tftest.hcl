...
run "minimal_request" {
  ...
  assert {
    condition     = msgraph_resource.this.url == "auditLogs/customSecurityAttributeAudits"
    error_message = "The collection URL must include parent identifiers and exclude the API-version prefix."
  }

  assert {
    condition     = alltrue([for key in ["activityDateTime", "activityDisplayName", "additionalDetails", "category", "correlationId", "initiatedBy", "loggedByService", "operationType", "result", "resultReason", "targetResources", "userAgent"] : !contains(keys(msgraph_resource.this.body), key)])
...
run "invalid_enum" {
  ...
  variables {
    result = "__graphmodules_invalid_enum__"
  }
