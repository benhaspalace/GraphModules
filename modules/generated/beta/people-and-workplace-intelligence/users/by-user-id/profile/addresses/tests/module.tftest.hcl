...
run "minimal_request" {
  ...
  assert {
    condition     = msgraph_resource.this.url == "users/test-parent-id/profile/addresses"
    error_message = "The collection URL must include parent identifiers and exclude the API-version prefix."
  }

  assert {
    condition     = alltrue([for key in ["allowedAudiences", "createdBy", "createdDateTime", "detail", "displayName", "geoCoordinates", "source", "inference", "isSearchable", "lastModifiedBy", "lastModifiedDateTime", "sources"] : !contains(keys(msgraph_resource.this.body), key)])
