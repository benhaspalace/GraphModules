...
run "minimal_request" {
  ...
  assert {
    condition     = msgraph_resource.this.url == "users/test-parent-id/profile/phones"
    error_message = "The collection URL must include parent identifiers and exclude the API-version prefix."
  }

  assert {
    condition     = alltrue([for key in ["allowedAudiences", "createdBy", "createdDateTime", "displayName", "source", "inference", "isSearchable", "lastModifiedBy", "lastModifiedDateTime", "number", "sources", "type"] : !contains(keys(msgraph_resource.this.body), key)])
